...
m1 <- lm(score ~ days_pre_hatch + birdID, data = aggData_filt)
summary(m1) # p-value = 3.899e-12
...
## For individuals with 3+ obs

m2 <- lm(score ~ days_pre_hatch, data = aggData_3)
summary(m2)

## result: no significant effect of date on aggression score (across all gulls) -- p = 0.289

m3 <- lm(score ~ days_pre_hatch + birdID, data = aggData_3)
summary(m3)

## results:
### What is the p-value for days_pre_hatch quantifying? (What about this model makes it significant?)
### significant effect of almost every individual on aggression score
