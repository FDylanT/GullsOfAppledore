...
gbbg_all <- read_csv("data/Kristen/aggData/behaviour_data.csv")
...
model1 <- lmer(response ~ obs_date_cal + location_cat + (1|bird_year),
               data = gbbg)
selected1 <- step(model1)
summary(selected1)
selected1$random
selected1$fixed

model1.1 <- lmer(latency ~ days_pre_hatch + obs_date_cal + location_cat + (1|bird_year),
             data = gbbg_lat)
selected1.1 <- step(model1.1)
selected1.1$random
selected1.1$fixed
