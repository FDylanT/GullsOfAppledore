...
gbbg <- gbbg_all %>%
  filter(!is.na(hatch1_date)) %>%
  filter(days_pre_hatch > 0) %>%
  filter(response >= 0) %>%
...
m1.1 <- lmer(latency ~ days_pre_hatch + hatch1_cal + (1|bird_year), data = gbbg)
summary(m1.1)
anova(m1.1)
...
m2.1 <- lmer(latency ~ days_pre_hatch + (1|bird_year) + (1|obs_num), data = gbbg)
...
plot(summary(m3)$residuals)
  # (but this does not)
m3.1 <- lmer(latency ~ days_pre_hatch + (1|bird_year) + (1|obs_num_cat), data = gbbg)

# this does not
m4 <- lmer(response ~ days_pre_hatch + hatch1_cal + (1|bird_year) + (1|obs_num_cat), data = gbbg)

m4.1 <- lmer(latency ~ days_pre_hatch + hatch1_cal + (1|bird_year) + (1|obs_num_cat), data = gbbg)
...
m5.1 <- lmer(latency ~ hatch1_cal + (1|bird_year) + (1|obs_num_cat), data = gbbg)
summary(m5.1)
```
...
h1.1 <- lmer(latency ~ days_pre_hatch + obs_num + (1|bird_year), data = habit)
summary(h1.1)
anova(h1.1)
...
ggplot(gbbg_sum, aes(x = days_pre_hatch, y = response)) +
  ...
  scale_x_reverse() +
  theme_bw() +
  geom_abline(slope = -summary(m3)$coefficients[2], intercept = summary(m3)$coefficients[1],
              col = "mediumpurple",
              linewidth = 1) +
  annotate("text",
           x = 3.5,
           y = 6.2,
           label = paste("slope =", round(-summary(m3)$coefficients[2], 3)), col = "mediumpurple") +
  annotate("text",
           x = 3.5,
           y = 6.15,
           label = paste("p =", round(summary(m3)$coefficients[2, 5], 7)), col = "mediumpurple")
```
...
g1.1 <- lmer(latency ~ days_pre_hatch + location_cat + (1|bird_year), data = gbbg)
summary(g1.1)
anova(g1.1)
